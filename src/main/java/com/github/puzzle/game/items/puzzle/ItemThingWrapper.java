...
public class ItemThingWrapper implements IModItem {

    final ItemThing parentItem;

    DataTagManifest manifest = new DataTagManifest();

    public ItemThingWrapper(Item parent) {
        if (parent == null) {
            parentItem = null;
        } else {
            parentItem = (ItemThing) parent;
        }
    }

    Identifier id = Identifier.fromString("puzzle-loader:item_thing_wrapper");
    ...
    public boolean isTool() {
        ...
        switch (parentItem.getID()) {
            case "base:stick":
                return true;
            default:
                return parentItem.getEffectiveBreakingSpeed(new ItemStack(parentItem, 1)) != 1.0F;
        }
    }
