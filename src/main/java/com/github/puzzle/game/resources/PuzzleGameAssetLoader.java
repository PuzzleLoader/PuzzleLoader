...
public class PuzzleGameAssetLoader {
    ...
    public static @Nullable FileHandle locateAsset(@NotNull ResourceLocation location) {
        ...
        if (vanillaLocationFile.exists()) {
            LOGGER.info("Loading " + AnsiColours.YELLOW + "\"{}\""+AnsiColours.WHITE+" from Base", location.name);
            return vanillaLocationFile;
        }

        vanillaLocationFile = Gdx.files.internal(location.name);
        if (vanillaLocationFile.exists()) {
            LOGGER.info("Loading " + AnsiColours.YELLOW + "\"{}\""+AnsiColours.WHITE+" from Base (Old)", location.name);
            return vanillaLocationFile;
        }
    ...
    public <T> void loadResource(@NotNull ResourceLocation location, Class<T> assetClass) {
        assetManager.load(location.toString(), assetClass);
    }

    public <T> T loadResourceSync(@NotNull ResourceLocation location, Class<T> assetClass) {
        assetManager.load(location.toString(), assetClass);
        return assetManager.finishLoadingAsset(location.toString());
    }

    public <T> T getResource(@NotNull ResourceLocation location, Class<T> assetClass) {
