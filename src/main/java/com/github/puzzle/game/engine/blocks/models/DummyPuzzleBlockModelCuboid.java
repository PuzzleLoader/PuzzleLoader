...
public class DummyPuzzleBlockModelCuboid
{
    ...
    public void initialize(IPuzzleBlockModel model, List<Face> allFaces) {
        for (final ObjectMap.Entry<String, Face> kf : this.faces) {

            boolean isValidFace = true;
            Face f = kf.value;
            if(f == null) {
                ServerGameLoader.LOGGER.warn("face '{}' of '{}@{}' is null, skipping", kf.key, model.getModelName(), model.getXZRotation());
                continue;
            }
