...
public class PuzzleBlockModelCuboid
{
    ...
    public void initialize(PuzzleBlockModel model, List<Face> allFaces) {
        for (final ObjectMap.Entry<String, Face> kf : this.faces) {

            boolean isValidFace = true;
            Face f = kf.value;
            if(f == null) {
                ServerGameLoader.LOGGER.warn("face '{}' of '{}@{}' is null, skipping", kf.key, model.modelName, model.rotXZ);
                continue;
            }
            ...
            switch (kf.key) {
                case "localNegX":
                    ...
                    f.midY2 = f.y2;
                    f.midZ2 = f.z1;
                    if (Constants.SIDE == Environment.CLIENT && t != null) {
            ...
            f.modelUvIdxB = CustomTextureLoader.makeUBOFloatsIdx(f.uB, f.vB, tmpNormal, tmpFaceNormal);
            CustomTextureLoader.setNormal(tmpNormal, f.x2, f.y2, f.z2);
            f.modelUvIdxC = CustomTextureLoader.makeUBOFloatsIdx(f.uC, f.vC, tmpNormal, tmpFaceNormal);
            CustomTextureLoader.setNormal(tmpNormal, f.midX2, f.midY2, f.midZ2);
            f.modelUvIdxD = CustomTextureLoader.makeUBOFloatsIdx(f.uD, f.vD, tmpNormal, tmpFaceNormal);
